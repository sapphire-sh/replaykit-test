...
namespace SA.iOS.GameKit.Internal
{
    ...
    internal class ISN_GKNativeAPI : ISN_Singleton<ISN_GKNativeAPI>, ISN_iGKAPI
    {
        ...
        #if API_ENABLED
        [DllImport(k_DllName)] static extern void _ISN_AuthenticateLocalPlayer(IntPtr callback, IntPtr didModifySavedGameCallback, IntPtr hasConflictingSavedGamesCallback);
        [DllImport(k_DllName)] static extern void _ISN_GKLocalPlayer_GenerateIdentityVerificationSignatureWithCompletionHandler(IntPtr callback);
        [DllImport(k_DllName)] static extern string _ISN_GetGKLocalPlayerInfo();
        [DllImport(k_DllName)] static extern void _ISN_GKGameCenterViewControllerShow(string data, IntPtr callback);

        //GKPlayer
        [DllImport(k_DllName)] static extern void _ISN_GKPlayer_LoadPhotoForSize(string playerId, int size, IntPtr callback);

        //Saved Games
        [DllImport(k_DllName)] static extern void _ISN_GKLocalPlayer_FetchSavedGames(IntPtr callback);
        [DllImport(k_DllName)] static extern void _ISN_GKLocalPlayer_SaveGameData(string name, string data, IntPtr callback);
        [DllImport(k_DllName)] static extern void _ISN_GKLocalPlayer_DeleteSavedGame(string name, string uniqueId, IntPtr callback);
        [DllImport(k_DllName)] static extern void _ISN_GKLocalPlayer_LoadGameData(string name, string uniqueId, IntPtr callback);
        [DllImport(k_DllName)] static extern void _ISN_GKLocalPlayer_ResolveSavedGames(string jsonContent, IntPtr callback);


        //Achievements
        [DllImport(k_DllName)] static extern void _ISN_GKAchievement_LoadAchievements(IntPtr callback);
        [DllImport(k_DllName)] static extern void _ISN_GKAchievement_ResetAchievements(IntPtr callback);
        [DllImport(k_DllName)] static extern void _ISN_GKAchievement_ReportAchievements(string contentJSON, IntPtr callback);

        //Leaderboards
        [DllImport(k_DllName)] static extern void _ISN_GKLeaderboard_LoadLeaderboards(IntPtr callback);
        [DllImport(k_DllName)] static extern void _ISN_GKLeaderboard_LoadScores(string leaderboardJSON, IntPtr callback);
        [DllImport(k_DllName)] static extern void _ISN_GKLeaderboard_ReportScore(string scoresJSON, IntPtr callback);

        #endif

        private readonly SA_Event<ISN_GKSavedGameSaveResult> m_DidModifySavedGame = new SA_Event<ISN_GKSavedGameSaveResult>();
        private readonly SA_Event<ISN_GKSavedGameFetchResult> m_HasConflictingSavedGames = new SA_Event<ISN_GKSavedGameFetchResult>();
        ...
        {
            get { return m_DidModifySavedGame; }
        }

        public SA_iEvent<ISN_GKSavedGameFetchResult> HasConflictingSavedGames
        {
            get { return m_HasConflictingSavedGames; }
        }
