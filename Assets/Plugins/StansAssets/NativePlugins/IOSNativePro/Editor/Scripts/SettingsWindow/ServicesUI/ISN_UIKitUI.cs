...
namespace SA.iOS
{
    ...
    {
        private readonly GUIContent PhotoLibraryUsageDescription = new GUIContent("Photo Library Usage Description[?]:", "Describes the reason that the app accesses the device’s photo library. When the system prompts the user to allow access, this string is displayed as part of the alert.");
        private readonly GUIContent CameraUsageDescription = new GUIContent("Camera Usage Description[?]:", "Describes the reason that the app accesses the device’s camera. When the system prompts the user to allow access, this string is displayed as part of the alert.");
        private readonly GUIContent PhotoLibraryAddUsageDescription = new GUIContent("Photo Library Add Usage Description[?]:", "Describes the reason for the app to add content to the device’s photo library. When the system prompts the user to allow access, this string is displayed as part of the alert.");
        private readonly GUIContent MicrophoneUsageDescription = new GUIContent("Microphone Usage Description[?]:", "Describes the reason that the app accesses the device’s microphone. When the system prompts the user to allow access, this string is displayed as part of the alert.");
        ...
        {
            get { return "Construct and manage a graphical, event-driven user interface for your app."; }
        }

        protected override Texture2D Icon 
        {
            get { return SA_EditorAssets.GetTextureAtPath(ISN_Skin.ICONS_PATH + "UIKit_icon.png"); }
        }

        public override SA_iAPIResolver Resolver 
        {
            get { return ISN_Preprocessor.GetResolver<ISN_UIKitResolver>(); }
        }
        ...
        protected override void OnServiceUI() 
        {
            using (new SA_WindowBlockWithSpace(new GUIContent("Protecting the User's Privacy"))) 
            {
                ...
                if (ISN_Settings.Instance.CameraUsageDescriptionEnabled) 
                {
                    ...
                    {
                        ISN_Settings.Instance.CameraUsageDescription = EditorGUILayout.TextArea(ISN_Settings.Instance.CameraUsageDescription, SA_PluginSettingsWindowStyles.TextArea, GUILayout.Height(textAreaHeight));
                    }
                }
                ...
                if (ISN_Settings.Instance.PhotoLibraryUsageDescriptionEnabled) 
                {
                    using (new SA_GuiIndentLevel(1)) 
                    {
                        ISN_Settings.Instance.PhotoLibraryUsageDescription = EditorGUILayout.TextArea(ISN_Settings.Instance.PhotoLibraryUsageDescription, SA_PluginSettingsWindowStyles.TextArea, GUILayout.Height(textAreaHeight));
                    }
                }
                ...
                if (ISN_Settings.Instance.PhotoLibraryAddUsageDescriptionEnabled)
                {
                    using (new SA_GuiIndentLevel(1)) 
                    {
                        ISN_Settings.Instance.PhotoLibraryAddUsageDescription = EditorGUILayout.TextArea(ISN_Settings.Instance.PhotoLibraryAddUsageDescription, SA_PluginSettingsWindowStyles.TextArea, GUILayout.Height(textAreaHeight));
                    }
                }
                ...
                if (ISN_Settings.Instance.MicrophoneUsageDescriptionEnabled) 
                {
                    using (new SA_GuiIndentLevel(1)) 
                    {
                        ISN_Settings.Instance.MicrophoneUsageDescription = EditorGUILayout.TextArea(ISN_Settings.Instance.MicrophoneUsageDescription, SA_PluginSettingsWindowStyles.TextArea, GUILayout.Height(textAreaHeight));
                    }
                }
            }
            ...
            using (new SA_WindowBlockWithSpace(new GUIContent("Allowed schemes to query"))) 
            {
                SA_EditorGUILayout.ReorderablList(ISN_Settings.Instance.ApplicationQueriesSchemes,
                  ...
                  () => 
                  {
                      ISN_UIUrlType newUlr = new ISN_UIUrlType("url_scheme");
