...
namespace SA.Foundation.Async
{
    public static class SA_Coroutine
    {
        ...
        /// </summary>
        public static Coroutine WaitForSecondsRealtime(float seconds, Action action)
        {
            return SA_InternalCoroutine.Instance.StartInstruction(new WaitForSecondsRealtime(seconds), action);
        }
        ...
        private class SA_InternalCoroutine : SA_Singleton<SA_InternalCoroutine>
        {
            ...
            public Coroutine StartInstruction(CustomYieldInstruction instruction, Action action) {
                return StartCoroutine(RunActionAfterInstruction(instruction, action));
            }
